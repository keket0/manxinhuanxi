...
### 1. tavily-search
- 状态：可用（当前实际主力是 `liang-tavily-search`）
- 价值：高质量网页搜索，适合文档、新闻、资料检索
- 优先场景：需要更结构化、更适合模型消费的搜索结果
- 依赖：`TAVILY_API_KEY`（已配置）
- 备注：`tavily-search` 这个 exact slug 当前装不上，但 `liang-tavily-search` 与 `liang-tavily-extract` 已实测跑通，可视为能力已到位
...
- 依赖：无额外 key
- 备注：另有 `weather-1-0-0`，但与 `weather` 高度重复，主用 `weather` 即可

## 二、干活型

### 4. cn-ecommerce-search
- 状态：半可用，旧版不可战斗，当前主力应切到 `cn-ecommerce-search-v2`
- 价值：搜索中国 8 大电商平台商品信息
- 优先场景：找商品、看商品链接、比价、找供应商
- 支持平台：淘宝、天猫、京东、拼多多、1688、AliExpress、抖音、小红书
- 依赖：旧版文档依赖 `@shopmeagent/cn-ecommerce-search-mcp`
- 备注：旧版依赖包当前公开 npm `404`，不可继续当主力；当前实战应以 `cn-ecommerce-search-v2` + `agent-browser` + 外部搜索组合为主
...
### 6. proactive-agent
- 状态：已安装，可学习，适合作为长期运行方法论
- 价值：提供更完整的主动代理架构，包括 WAL、Working Buffer、Compaction Recovery、安全硬化、统一搜索与 cron 分层
- 优先场景：长任务、上下文容易压缩的会话、需要持续主动推进的环境
- 依赖：无额外 key / 无额外二进制
- 备注：不建议机械照搬其整套 workspace 结构，应吸收方法论而不是直接覆盖现有体系
...
### 8. self-improving-proactive-agent
- 状态：可用
...
### 15. find-skills
- 状态：目标能力已覆盖，但 exact slug 当前装不上
- 价值：技能发现
- 优先场景：想找某类技能、查有没有现成 skill
- 依赖：原文档依赖 `npx skills`
- 备注：当前环境下更推荐直接使用 `find-skills-for-clawhub` + `openclaw-find-skills`，`find-skills` 本名安装返回 `Skill not found`

### 16. openclaw-find-skills
- 状态：可用
- 价值：补充型找技能技能，适合继续扩展技能发现能力
- 优先场景：继续搜技能、查替代 skill、补充生态导航
- 依赖：按技能文档执行
...
- `weather` → 已可用，主用 `weather`
- `Agent Browser` → 已可用，主力为 `agent-browser-core` + `agent-browser-clawdbot`
- `claw-ds-generator` → 仍不可安装，且更像不存在 / 下架 / 名字不对
- `cn-ecommerce-search` → 旧版已装但不可战斗，主力应切到 `cn-ecommerce-search-v2`
- `duckduckgo-search` → 已可用
- `find-skills` → exact slug 装不上，但能力已由 `find-skills-for-clawhub` + `openclaw-find-skills` 覆盖
- `openai-tts` → 已装，半可用
- `proactive-agent` → 已装，且比 `proactive-agent-lite` 更完整
- `self-improvement` → 已可用
- `tavily-search` → exact slug 装不上，但能力已由 `liang-tavily-search` 覆盖
